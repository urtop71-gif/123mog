...
export default function MealList({ meals }: MealListProps) {
  ...
  if (meals.length === 0) {
    return (
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-8 text-center mb-8">
        <p className="text-gray-400">{t.dashboard.empty}</p>
        <p className="text-sm text-gray-300 mt-1">{t.dashboard.emptyHint}</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4 mb-8">
      <h2 className="text-lg font-semibold text-gray-900">{t.dashboard.mealList}</h2>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {meals.map((meal, index) => {
          const config = MEAL_TYPE_CONFIG[meal.mealType] || { emoji: "🍽️", color: "border-l-gray-400", label: meal.mealType };
          return (
            <div key={meal.mealType + "-" + index} className={`bg-white rounded-xl shadow-sm border border-gray-100 border-l-4 ${config.color} p-4`}>
              <div className="flex items-center justify-between mb-3">
                <h3 className="font-semibold text-gray-900">{config.emoji} {config.label}</h3>
                ...
                <ul className="space-y-2">
                  {meal.items.map((item) => (
                    <li key={item.id} className="flex justify-between items-center text-sm">
                      ...
                      <span className="text-gray-400">{item.quantity}{item.unitName} · {Math.round(item.totalCalories)} kcal</span>
                    </li>
                  ))}
                </ul>
